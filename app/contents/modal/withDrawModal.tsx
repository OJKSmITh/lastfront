...
export const WithdrawPairModalContent =({token}:IWithdrawPairModalContent)=>{
    ...
    useEffect(()=>{
        ...
        if(token ==="ETH"){
          const fetchData = async () => {
            const {amount1:{_hex:convert1},amount2:{_hex:convert2}} = await calcLp(signerInstance,EthLp!,differLpamount)
            const bigNumber1 = ethers.BigNumber.from(convert1)
            const bigNumber2 = ethers.BigNumber.from(convert2)
            const converted1 = bigNumber1.div(ethers.constants.WeiPerEther).toNumber();
            const converted2 = bigNumber2.div(ethers.constants.WeiPerEther).toNumber();
          ...
          fetchData()
            ...
            const fetchData = async () => {
                const {amount1:{_hex:convert1},amount2:{_hex:convert2}} = await calcLp(signerInstance,ArbLp!,differLpamount)
                const bigNumber1 = ethers.BigNumber.from(convert1)
                const bigNumber2 = ethers.BigNumber.from(convert2)
                const converted1 = bigNumber1.div(ethers.constants.WeiPerEther).toNumber();
                const converted2 = bigNumber2.div(ethers.constants.WeiPerEther).toNumber();
            ...
            const fetchData = async () => {
                const {amount1:{_hex:convert1},amount2:{_hex:convert2}} = await calcLp(signerInstance,UsdtLp!,differLpamount)
                const bigNumber1 = ethers.BigNumber.from(convert1)
                const bigNumber2 = ethers.BigNumber.from(convert2)
                const converted1 = bigNumber1.div(ethers.constants.WeiPerEther).toNumber();
                const converted2 = bigNumber2.div(ethers.constants.WeiPerEther).toNumber();
